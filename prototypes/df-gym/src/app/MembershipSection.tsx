...
export default function MembershipSection() {
  ...
  return <section className={styles.section} id="membresia" aria-labelledby="membership-title"><div className={styles.intro}><span>Empezá con claridad</span><h2 id="membership-title">Tu lugar en Distrito Fuerza.</h2><p>Contanos qué buscás y recibí la información para sumarte. Esta muestra no inventa precios ni hace cobros.</p></div>{sent ? <div className={styles.success} role="status"><strong>Solicitud simulada</strong><p>Gracias, {name}. En la versión real, el equipo te contactaría por el canal indicado.</p><button type="button" onClick={() => setSent(false)}>Volver a consultar</button></div> : <form className={styles.form} onSubmit={submit} noValidate><label htmlFor="member-interest">Me interesa</label><select id="member-interest" value={interest} onChange={(event) => setInterest(event.target.value)}><option value="">Elegí una opción</option>{options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label htmlFor="member-name">Nombre</label><input id="member-name" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" /><label htmlFor="member-contact">WhatsApp o email</label><input id="member-contact" value={contact} onChange={(event) => setContact(event.target.value)} autoComplete="email" />{error && <p className={styles.error} role="alert">{error}</p>}<button className={styles.submit} type="submit">Quiero hacerme socio</button><small>Demo: no hay cobro ni alta real conectada.</small></form>}</section>;
}
