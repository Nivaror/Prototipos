...
/**
 * Which night's billing is currently on. A venue that closes at 03:00 is still
 * running Friday's night at 01:30 on Saturday, so before yesterday's shift ends
 * the night belongs to yesterday.
 */
export function nightWeekday(schedule: Schedule, weekday: number, minutes: number) {
  const yesterday = schedule[(weekday + 6) % 7] ?? [];
  const carriesOver = yesterday.some((s) => s.to > 1440 && minutes + 1440 < s.to);
  return carriesOver ? (weekday + 6) % 7 : weekday;
}

/** Half-hour slots inside a shift, wrapping past midnight via fmt(). */
export function slotsOf(shifts: Shift[], step = 30) {
  const slots: string[] = [];
  for (const shift of shifts) {
    for (let m = shift.from; m <= shift.to - step; m += step) slots.push(fmt(m));
  }
  return slots;
}

/** ISO date `days` after `iso`, kept in local calendar terms only. */
